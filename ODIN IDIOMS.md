...
- `items[:]` creates a slice view over existing storage. It does not transfer
  ownership of dynamic-array storage.
- If finalized slice storage must own exact-length memory, allocate a slice of
  the exact length and copy into it.
- If ownership of dynamic-array storage matters, keep the dynamic array with
  the owner.
