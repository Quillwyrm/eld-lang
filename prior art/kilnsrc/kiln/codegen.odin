...
emit_struct_get :: proc(proto_state: ^ProtoState, dst, struct_slot, field_slot: int) {
    record_slots(proto_state, dst, struct_slot, field_slot)

    inst := u32(InstABC{ op= .STRUCT_GET, a= u8(dst), b= u8(struct_slot), c= u8(field_slot) })
    emit_inst(proto_state, inst)
}

emit_struct_set :: proc(proto_state: ^ProtoState, struct_slot, field_slot, src: int) {
    record_slots(proto_state, struct_slot, field_slot, src)

    inst := u32(InstABC{ op= .STRUCT_SET, a= u8(struct_slot), b= u8(field_slot), c= u8(src) })
    emit_inst(proto_state, inst)
}
...
retarget_last_result :: proc(proto_state: ^ProtoState, old_dst, new_dst: int) -> bool {
    ...
    case .INDEX_GET, .ARRAY_GET_CONST, .MAP_GET_CONST, .STRUCT_GET_CONST, .STRUCT_GET,
