...
Opcode :: enum u8 {
    ...
    STRUCT_GET,       // ABC: A=dst,    B=struct, C=field_name_slot
    STRUCT_SET,       // ABC: A=struct, B=field_name_slot, C=src
...
struct_field_assigned_value :: proc(field_spec: Value, assigned_value: Value) -> (field_value: Value, assignment_matches: bool) {
    field_value = assigned_value

    switch spec in field_spec {
    case bool:
        _, assignment_matches = assigned_value.(bool)

    case i64:
        _, assignment_matches = assigned_value.(i64)

    case f64:
        if _, value_is_float := assigned_value.(f64); value_is_float {
            assignment_matches = true
        } else if value_int, value_is_int := assigned_value.(i64); value_is_int {
            field_value = Value(f64(value_int))
            assignment_matches = true
        }

    case ^Object:
        value_object, value_is_object := assigned_value.(^Object)
        if value_is_object {
            switch spec.kind {
            case .STRING:
...
run_proto :: proc(state: ^State, proto: ^Proto) -> (result: Value, err: string) {
    ...
    for {
        ...
        case .STRUCT_GET:
            ...
            dst := slot_base + int(inst.a)
            struct_slot := slot_base + int(inst.b)
            field_slot := slot_base + int(inst.c)

            field_header, field_is_object := state.slots[field_slot].(^Object)
            if !field_is_object || field_header.kind != .STRING {
                frame.instruction_index = pc
                return Value{}, runtime_error(fmt.tprintf("invalid struct field read; expected field name `string`, got `%s`", value_type_to_string(state.slots[field_slot])))
            }
            field_name := cast(^StringObject)field_header
        ...
        case .STRUCT_SET_CONST:
            ...
            stored_value, assignment_matches := struct_field_assigned_value(field_spec, value)
        ...
        case .STRUCT_SET:
            inst := InstABC(word)
            struct_slot := slot_base + int(inst.a)
            field_slot := slot_base + int(inst.b)
            value_slot := slot_base + int(inst.c)

            field_header, field_is_object := state.slots[field_slot].(^Object)
            if !field_is_object || field_header.kind != .STRING {
                frame.instruction_index = pc
                return Value{}, runtime_error(fmt.tprintf("invalid struct field assignment; expected field name `string`, got `%s`", value_type_to_string(state.slots[field_slot])))
            }
            field_name := cast(^StringObject)field_header
