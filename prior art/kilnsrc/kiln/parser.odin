...
lower_expr_desc :: proc(proto_state: ^ProtoState, expr: ExprDesc, dst_slot: int) {
    ...
    case ExprStructField:
        emit_struct_get(proto_state, dst_slot, desc.struct_slot, desc.field_slot)
    ...
    if Parser.current_token.kind != .RIGHT_BRACE {
        for {
            ...
            if field_const <= MAX_ABC_OPERAND {
                ...
                field_slot := claim_temp_slot(proto_state)
                if Parser.failed { return ExprInvalid{} }

                emit_load_const(proto_state, field_slot, field_const)
                emit_struct_set(proto_state, dst_slot, field_slot, value_slot)
            }
...
parse_field_postfix :: proc(proto_state: ^ProtoState, left: ExprDesc) -> ExprDesc {
    ...
    emit_load_const(proto_state, field_slot, field_const)
    return ExprStructField{left_slot, field_slot}
}
...
resolve_assign_target :: proc(proto_state: ^ProtoState, source_token: Token, target: ExprDesc) -> ExprDesc {
    ...
    case ExprLocalBinding, ExprFileBinding, ExprGlobalBinding, ExprIndex, ExprArrayIndexConst, ExprMapIndexConst, ExprStructFieldConst, ExprStructField:
        return target
...
set_assign_target :: proc(proto_state: ^ProtoState, src_slot: int, target: ExprDesc) {
    ...
    case ExprStructField:
        emit_struct_set(proto_state, t.struct_slot, t.field_slot, src_slot)
...
parse_simple_stmt :: proc(proto_state: ^ProtoState) {
    ...
    if lhs_count == 1 {
        ...
        case ExprStructFieldConst, ExprStructField:
