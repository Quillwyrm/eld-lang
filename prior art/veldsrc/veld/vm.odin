...
// Appends Veld display fragments for value.
// Composite values should recurse through this proc.
append_value_string :: proc(parts: ^[dynamic]string, value: Value) {
	switch v in value {
	case i64:
		append(parts, fmt.tprintf("%d", v))

	case f64:
		if v == f64(i64(v)) {
			append(parts, fmt.tprintf("%.1f", v))
		} else {
			append(parts, fmt.tprintf("%g", v))
		}
...
// Returns owned text made by concatenating the display rendering of lhs and rhs.
// The caller owns the returned string.
concat_value_text :: proc(lhs, rhs: Value) -> string {
	parts := make([dynamic]string)
	append_value_string(&parts, lhs)
	append_value_string(&parts, rhs)
...
write_value :: proc(value: Value) {
	parts := make([dynamic]string)
	append_value_string(&parts, value)
...
run_code :: proc(vm: ^VM, code: ^Code) -> string {
	Active_VM = vm
	vm.error_string = ""
	...
	for ip := 0; ip < len(code.bytecode); ip += 1 {
		...
		case .ADD:
			...
			case i64:
				...
				case ^ObjectHeader:
					if r.kind != .STRING {
						runtime_error("+ expected numbers or at least one string")
						return Active_VM.error_string
					}
			...
			if string_concat {
				text := concat_value_text(lhs, rhs)
				object := new(StringObject)
				object.header.kind = .STRING
				object.text = text
				push_value(Value(&object.header))
			}
		...
		case .SUB:
			...
			case i64:
				...
				case ^ObjectHeader:
					runtime_error("- expected numbers")
					return Active_VM.error_string
				}
